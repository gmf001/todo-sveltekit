<script lang="ts">
	import { deleteTodo, editTodo, toggleTodo, type Todo } from '$lib/store/TodoStore';
...
<div class="flex w-full items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
	<div class="flex flex-1 w-full max-w-lg items-center justify-start">
		...
		<input
			...
			value={todo.text}
			on:input={(e) => editTodo(todo.id, e.currentTarget.value)}
		...
		/>
		<span
			class="{completeClass} ml-5 hidden rounded-full text-sm py-0.5 px-2 font-normal text-gray-five md:block"
		>
	...
	<button
		class="group flex ml-4 items-center justify-center rounded-md bg-cream-four p-2 hover:bg-steel-one focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-five"
